 ...
 * 比 agent 在 Bash 里直接跑多两样：KITE_BASE 由 Kite 给，取会话分支和主线的分叉点，会话里已经提交的改动
 * 也算进受影响的范围；结果发成 check 事件，App 可以直接显示。
 */
...
export async function runCheck(o: { main: string; worktree: string; all?: boolean; signal?: AbortSignal; timeoutMs?: number }): Promise<CheckResult> {
...
function report(r: CheckResult): string {
  ...
  if (r.stopped === 'timeout') return ['检查超时，已停止。', out].filter(Boolean).join('\n');
  if (r.stopped === 'aborted') return '检查被打断。';
  return out || (r.ok ? '检查通过。' : `检查没通过，退出码 ${r.code}，没有输出。`);
}

export function checkTool(o: { main: string; worktree: string; onResult(r: CheckResult): void }) {
  return tool(
    'check',
    ...
    async ({ all }, extra) => {
      const r = await runCheck({ main: o.main, worktree: o.worktree, all, signal: (extra as { signal?: AbortSignal }).signal });
      o.onResult(r);
