/**
 * 经 HTTP 驱动本进程里的 kited，会话起真实的 Claude Code，模型换成假端点：D1、D6、D7。
 */
...
test('一条完整的会话：第一条消息到达 agent，「会话开始」是最早的快照，一批并行工具调用一枚快照，回合结束关闭，再发消息续接同一个原生会话，有没合回的改动时归档要 force，归档后工作树和分支删掉、快照留下、不能再发消息', async () => {
  k = startKited();
  const kk = k;
...
test('agent 正在工作时，采纳和回退都被拒绝（409）', async () => {
  k = startKited();
  const kk = k;
  const repo = newRepo(kk.root, 'proj', { 'a.txt': 'a\n' });
  const p = await registerProject(kk, repo);
