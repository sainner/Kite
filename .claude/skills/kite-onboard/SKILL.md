...
5. **输出只报结论**。通过时一行；没通过时只列失败项和超预算项，完整日志写进文件并给出路径，不把几千行测试输出塞进 agent 的上下文。日志和测试报告每次写进一个新的临时目录：几个会话、主 agent 和子 agent 可能同时跑检查，写在固定位置会互相覆盖。
...
写测试和写实现分开：主 agent 写实现，test-writer 只照需求写测试，看不到实现，所以测试检验的是需求，不会复述实现。哪些需求要测由主 agent 定：它看得到代码，只交看代码确认不了、要跑起来才知道对不对的。两边可以并行，前提是动手前先把需求和接口写下来。测试规则只在这个子 agent 工作时加载，平时不占上下文。
