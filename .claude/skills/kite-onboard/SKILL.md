...
5. **输出只报结论**。通过时一行；没通过时只列失败项和超预算项，完整日志写进文件并给出路径，不把几千行测试输出塞进 agent 的上下文。日志写在哪由调用方定：Kite 的 check 工具经 `KITE_LOG_DIR` 给一个目录；手动跑时没有这个变量，每次自己建一个临时目录，不写在固定位置，免得同时跑的检查互相覆盖。
