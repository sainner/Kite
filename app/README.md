...
布局：Mac 上左边侧边栏（上面是列表，底部是 action 区；拖右边的缝调宽度，拖到很窄收起，⌃⌘S 切换），右边一组卡片，按住卡片标题栏拖出去，它脱离布局变成跟着指针的圆，落到别的卡片上时用占位预览放下后的排布，排布和拖放见 `Tiles.swift`；iPhone 上一个窗口铺满屏幕，窗口顶部的页签切换内容，侧边栏和 action 栏从边上拉出来，见 `PhoneLayout.swift`。尺寸和颜色集中在 `Theme.swift`。
